...
document
  ...
  ?.addEventListener("submit", async (e) => {
    ...
    const username = (
      document.getElementById("usernameEdit") as HTMLInputElement
    ).value;
    const email = (document.getElementById("emailEdit") as HTMLInputElement)
      .value;
    const password = (
      document.getElementById("passwordEdit") as HTMLInputElement
    ).value;
...
document.getElementById("deleteBtn")?.addEventListener("click", async () => {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Something went wrong trying to delete user: ${response.status}`
      );
    }
